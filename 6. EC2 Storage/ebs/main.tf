...
resource "aws_ebs_volume" "test_volume_chap_6" {
  availability_zone = local.availability_zone
  size = 2
}

resource "aws_volume_attachment" "volume_attachment_chap_6_vol_to_ec2" {
  volume_id = aws_ebs_volume.test_volume_chap_6.id
  instance_id = aws_instance.chap-6-ec2.id
  device_name = "/dev/sdb"
}